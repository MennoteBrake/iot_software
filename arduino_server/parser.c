  ...
  case EXPECT_METHOD:
    if (tok.type == WORD) {
      ...
      if (strncmp(tok.value, "GET", TOKEN_ARR_LEN) == 0) {
        ...
        request->method = POST;
        return EXPECT_WS_1;
      } else if (strncmp(tok.value, "PUT", TOKEN_LEN - 1) ==
                 0) {
        printf("PUT \n");
        request->method = PUT;
        return EXPECT_WS_1;
...
void checkTarget(const struct token tok,
  ...
  if (request->method == GET &&
      strncmp(tok.value, "/sensors/1/avg", TOKEN_ARR_LEN) ==
          0) {
    // printf("AVG SENSOR1  \n");
    request->target = AVG1;
    ...
    printf("req target %i \n", request->target);
             ...
             strncmp(tok.value, "/sensors/1/avg",
                     TOKEN_ARR_LEN) == 0) {
  ...
  //   }
  //    else if(request->method == GET &&
  //    strncmp(tok->value  //    else if(strncmp(request->method == GET &&
, "/sensors/2/stdev",
  //    TOKEN_ARR_LEN) == 0){
